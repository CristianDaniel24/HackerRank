...
class Result {
    ...
    public static void countApplesAndOranges(int s, int t, int a, int b, List<Integer> apples, List<Integer> oranges) {
        ...
        for (int i = 0; i < apples.size(); i++) {
            int posicionFinalApples = a + apples.get(i);
            if (posicionFinalApples >= s && posicionFinalApples <= t) {
                contApples++;
            }
        }
        for (int i = 0; i < oranges.size(); i++) {
            int posicionFinalOrange = b + oranges.get(i);
            if (posicionFinalOrange >= s && posicionFinalOrange <= t) {
                contOrange++;
            }
        }
        System.out.println(contApples);
        System.out.println(contOrange);
    }
}
